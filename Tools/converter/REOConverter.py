...
def convert_reo_to_obj(in_file, out_file):
    ...
    while (linecounter < len(read_data)):
        ...
        # set metadata
            ...
            materialcount = int(read_data[linecounter].replace("materials ", "", 1).replace("\n", ""))
            materials = []
            ...
            REO.transform = [read_data[linecounter + 1].replace("\n", "").split(" "),
                             read_data[linecounter + 2].replace("\n", "").split(" "),
                             read_data[linecounter + 3].replace("\n", "").split(" ")]
            linecounter += 3
    ...
    if (withTextures):
        # Build the mtl file
        with open(out_file[:-3] + "mtl", 'w') as f:
    ...
    with open(out_file, 'w') as f:
        ...
        if (withTextures):
            ...
            for entry in REO.faces:
                withtexid = []
                a = list(reversed(entry[1]))
                # associate the vertices with their corresponding vt entry id
                for index, vert in enumerate(reversed(entry[0])):
                    withtexid.append(str(int(vert) + 1) + '/' + str(int(a[index]) + 1))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='REO to OBJ converter')
    parser.add_argument('--input', '-i', dest='inDir', required=True, nargs=1,
                        help='Input directory with REO files to process')
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.reo'):
            print('Processing %s' % filename)
            convert_reo_to_obj(os.path.join(input_dir, filename), os.path.join(output_dir, filename[:-3] + 'obj'))
