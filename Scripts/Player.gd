...
func _ready():
	...
	weaponScene = load("res://Scenes/Weapon/Stick.tscn").instance()
	$RH_slot.add_child(weaponScene)
	weaponNodeName = weaponScene.name
	weapon = get_node("RH_slot/" + weaponNodeName)
...
func _process(delta):
	...
	# Moving weapon
#	if weapon.get_translation().length() > 2:
#		weapon.set_translation(Vector3(0,0,0))
#	elif weapon.get_translation().length() < 1:
#		weapon.move_and_slide(Vector3(0,0,0), Vector3(0,1,0))
#	else:
#		weapon.move_and_slide(-100 * get_translation(), Vector3(0,1,0))
	...
	#	print(current - dupRoll)
		...
		if get_slide_count() > 0:
			...
			if collision.collider is RigidBody:
				collision.collider.apply_impulse(collision.position, -collision.normal / 10)
